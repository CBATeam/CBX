...
namespace cbx {
    ...
    int32_t _compressed_base::_decompress_safe(std::istream &in, uint32_t expected_size_) {
        ...
        save_pos = in.tellg();
        buffer = new uint8_t[expected_size_ + 1024];

        in.read((char*) buffer, expected_size_ + 1024);

        // I do not understand why this is needed. Use it and avoid warnings through (void)
        std::streamsize input_size = in.gcount();
        (void)input_size;
        ...
        _data = std::unique_ptr<uint8_t[]>(
                new uint8_t[expected_size_ + (expected_size_ % 8)]);
        result = _mikero_lzo1x_decompress_safe(buffer, _data.get(),
                expected_size_);
        if (result < 0) {
            delete[] buffer;
    ...
    int _compressed_base::_mikero_lzo1x_decompress_safe(const uint8_t *in,
            uint8_t *out, uint32_t OutLen) {
        // The following variables were marked with the keyword register
        uint8_t *op;
        const uint8_t *ip;
        size_t t;
        const uint8_t *m_pos;
...
#define READ_CHAR2(var) in.read((char *)&temp, 1); var = (unsigned char)temp
    bool _compressed_base::_lzss_decompress(std::istream &in, long lensb) {
        ...
        if (lensb <= 0)
            return true;
        std::unique_ptr<uint8_t[]> temp_buf = std::unique_ptr<uint8_t[]>(
                new uint8_t[4113]);
